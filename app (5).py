...
def build_actuarial_report(df_psap, ibnr, ppna=None, pb=None, logger=None):
    psap_total = df_psap["psap"].sum() if "psap" in df_psap.columns else 0
    paid_total = df_psap["paid_amount"].sum() if "paid_amount" in df_psap.columns else 0
    ultimate_total = df_psap["estimated_ultimate"].sum() if "estimated_ultimate" in df_psap.columns else 0

    total_provisions = psap_total + ibnr
    if ppna is not None:
        total_provisions += ppna
    if pb is not None:
        total_provisions += pb
    ...
    if psap_total > 0:
        commentaires.append("Le PSAP est positif : des sinistres restent à provisionner.")
    else:
        commentaires.append("Le PSAP est nul : aucun écart positif entre ultimate et payé.")

    if ibnr > 0:
        commentaires.append("L’IBNR est positif : une charge future est estimée sur les sinistres non encore déclarés.")
    else:
        commentaires.append("L’IBNR est nul ou faible : la charge future estimée est limitée.")

    if ppna is not None:
        commentaires.append("La PPNA représente la part de primes non acquises à la date de clôture.")

    if pb is not None:
        commentaires.append("La PB correspond à la participation aux bénéfices calculée sur les données disponibles.")
    ...
    bilan = pd.DataFrame({
        ...
        "valeur": [
            ...
            psap_total,
            ibnr,
            ppna if ppna is not None else 0,
            pb if pb is not None else 0,
            total_provisions
        ]
    })

    commentaires_df = pd.DataFrame({"commentaire": commentaires})
    logs_df = logger.get_logs() if logger is not None else pd.DataFrame()

    return {
        "bilan_actuariel": bilan,
        "commentaires_bilan": commentaires_df,
        "logs_audit": logs_df
    }
