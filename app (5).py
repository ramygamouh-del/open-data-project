...
st.set_page_config(page_title="Projet Actuariel", layout="wide")
st.title("Projet actuariel - Streamlit")

st.write("Importer les fichiers nécessaires puis lancer les calculs.")

# ==========================================
# Uploads
# ==========================================
fichier_sap = st.file_uploader("Importer le fichier SAP", type=["xlsx", "csv"], key="sap")
fichier_est = st.file_uploader("Importer le fichier Estimations", type=["xlsx", "csv"], key="est")
fichier_pb = st.file_uploader("Importer le fichier PB", type=["xlsx", "csv"], key="pb")
fichier_ppna = st.file_uploader("Importer le fichier PPNA", type=["xlsx", "csv"], key="ppna")

dev_factor = st.number_input("Facteur de développement IBNR", min_value=1.0, value=1.2, step=0.05)
...
if st.button("Lancer le traitement"):
    ...
    try:
        if fichier_sap is None or fichier_est is None:
            ...
            st.subheader("Résultats SAP / Estimations")
            col1, col2 = st.columns(2)
            with col1:
                st.metric("PSAP total", f"{df_psap['psap'].sum():,.2f}")
            with col2:
                st.metric("IBNR", f"{ibnr:,.2f}")

            st.dataframe(df_psap.head(), use_container_width=True)
            ...
            if fichier_ppna is not None:
                ...
                st.subheader("PPNA")
                st.metric("PPNA", f"{ppna:,.2f}")
                result_sheets["ppna_logs"] = logger.get_logs()
            ...
            if fichier_pb is not None:
                ...
                result_sheets["pb_logs"] = logger.get_logs()

            st.subheader("Journal d'audit")
            st.dataframe(logger.get_logs(), use_container_width=True)

            excel_bytes = dataframe_to_excel_bytes(result_sheets)

            st.download_button(
                label="Télécharger les résultats Excel",
                data=excel_bytes,
                file_name="resultats_actuariels.xlsx",
